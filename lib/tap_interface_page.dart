...
class _TapInterfacePageState extends State<TapInterfacePage> {
  ...
  void _initializeWakeWordService() {
    // Skip wake word initialization in offline mode
  ...
  Future<void> _announceViaBackend(
    ...
    try {
      ...
      // In offline mode, use local TTS directly — no backend calls
  ...
  Future<void> _playCustomAudio(String audioUrl) async {
    try {
      ...
      // In offline mode, try to play from local cache
      if (widget.isOfflineMode) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                ...
                children: [
                  ...
                  if (!_isAdminToolbarLocked) ...[
                    IconButton(
                      ...
                      onPressed: widget.isOfflineMode
                          ? () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Admin settings are unavailable in offline mode'),
                                ),
                              );
                            }
          ...
          if (widget.isOfflineMode) _buildOfflineIndicator(),
        ],
      ),
    );
  }
