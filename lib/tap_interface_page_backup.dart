  ...
  Future<void> _loadPictogram() async {
    ...
    try {
      ...
      if (allWordsAreSightWords) {
        ...
        // If not all words are sight words, proceed with normal pictogram loading
        // Only load pictograms if enablePictograms is true
        if (widget.enablePictograms) {
          ...
          // enablePictograms is false, don't load image
          if (mounted) {
            setState(() {
              _pictogramUrl = null;
              _isSightWord = false;
...
class _TapInterfacePageState extends State<TapInterfacePage> {
        ...
        (index) {
          if (index == (userSettings?.llmOptions ?? 10)) {
            return TapInterfaceButton(
              ...
              enableSightWords: userSettings?.enableSightWords ?? true,
            ...
            return TapInterfaceButton(
              ...
              onPressed: () => _handlePhraseOptionTap(phraseOption['fullText'] ?? ''),
              backgroundColor: Colors.green[50] ?? Colors.green.shade50,
              foregroundColor: Colors.black87,
              borderColor: Colors.green[300] ?? Colors.green.shade300,
              fontSize: 8,
  ...
  void _setupWakeWordCallbacks() {
    ...
    _wakeWordService!.onQuestion = (question) async {
      ...
      setState(() {
        ...
        _statusMessage = 'Question heard: "$question"';
      });
      
      // Show the question in speech bubble
      _showSpeechBubbleOverlay(question);
      
      // Acknowledge the question like the grid page does
      setState(() {
  ...
  void _startWakeWordListening() async {
    ...
    setState(() {
      ...
      _statusMessage = 'Say "${_getFormattedWakeWord()}" to ask a question...';
    });
  ...
  Future<void> _loadTapInterfaceConfig() async {
    ...
    try {
      ...
      if (config != null) {
        debugPrint('[TapInterface] 📋 Loaded tap config with ${config.buttons.length} categories:');
        for (var i = 0; i < config.buttons.length; i++) {
          final category = config.buttons[i];
  ...
  void _preloadCategoryImages() async {
    ...
    try {
      ...
      final futures = _tapConfig!.buttons.map((category) async {
        try {
          final result = await pictogramService.getPictogramResult(category.label);
  ...
  Future<void> _speakSpeechHistory() async {
    ...
    debugPrint('[TapInterface] _speakSpeechHistory called with autoClean=$autoClean, _speechHistory="$_speechHistory"');
    
    String textToSpeak = _speechHistory;
    
    if (autoClean) {
      debugPrint('[TapInterface] Auto Clean is enabled, starting text cleanup');
      // Clean the text first
      textToSpeak = await _cleanupText(_speechHistory);
  ...
  Future<void> _announceViaBackend(String text) async {
    try {
      ...
      await _announceViaBackendSimple(text, routing: 'system', preserveMicrophoneSession: true);
  ...
  Future<void> _announceViaBackendSimple(
    ...
    try {
      ...
      final voiceSettings = {
        ...
        // Include user's TTS preferences to ensure correct voice is used
        if (userSettings?.selectedTtsVoiceName != null && userSettings!.selectedTtsVoiceName.isNotEmpty) 
          'voice': userSettings.selectedTtsVoiceName,
        if (userSettings?.speechRate != null) 
          'speed': userSettings!.speechRate,
      };
      
      debugPrint('[TapInterface] TTS request with voice settings: $voiceSettings');
      
      // Request backend audio (same as grid page)
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Text(
              _pastSpeechHistory.isEmpty ? 'No speech history yet.' : _pastSpeechHistory.reversed.join('\n\n'),
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  // Status Message - takes up remaining space
