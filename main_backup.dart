...
// Standard HttpOverrides for normal network connectivity
class StandardHttpOverrides extends HttpOverrides {
  @override
  HttpClient createHttpClient(SecurityContext? context) {
    final client = super.createHttpClient(context);
    // Standard timeouts for normal networks
...
Future<T> retryNetworkOperation<T>(
  ...
  int maxRetries = 3, // Standard retry count
  Duration delay = const Duration(seconds: 1), // Quick retry
...
// Test network connectivity with known working sites
Future<void> testCruiseShipConnectivity() async {
  final testSites = [
  ...
  debugPrint('🧪 Testing cruise ship network with known working sites...');

  for (final testUrl in testSites) {
    try {
      debugPrint('🧪 Testing: $testUrl');
      final response = await cruiseShipSafeGet(testUrl);
      debugPrint('🧪 SUCCESS: $testUrl -> Status ${response.statusCode}');
  ...
  debugPrint('🧪 Network connectivity test completed');
}
...
void main() {
  ...
  try {
    ...
    SightWordService().initialize().then((_) {
...
class _UserSelectionPageState extends State<UserSelectionPage> {
  ...
  Future<void> _performDeleteProfile(String aacUserId) async {
    try {
      final response = await http.delete(
        Uri.parse('${EnvironmentConfig.apiBaseUrl}/api/account/delete-user'),
        headers: {
          'Authorization': 'Bearer ${widget.idToken}',
          'Content-Type': 'application/json',
        },
  ...
  Future<void> _performCopyProfile(
    ...
    try {
      final response = await http.post(
        Uri.parse('${EnvironmentConfig.apiBaseUrl}/api/account/copy-user'),
        headers: {
          'Authorization': 'Bearer ${widget.idToken}',
          'Content-Type': 'application/json',
        },
  ...
  Future<void> _createNewProfile() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${EnvironmentConfig.apiBaseUrl}/api/account/add-aac-user'),
        headers: {
          'Authorization': 'Bearer ${widget.idToken}',
          'Content-Type': 'application/json',
        },
  ...
  Future<void> _initializeFirebaseAsync() async {
    try {
      ...
      if (kIsWeb) {
        ...
        await Future.any([
          Future.delayed(const Duration(seconds: 15), () => throw TimeoutException('Firebase init timeout')),
      ...
      if (!kIsWeb) {
        try {
          await Future.any([
            Future.delayed(const Duration(seconds: 5), () => throw TimeoutException('Auth persistence timeout')),
  ...
  Future<void> _checkFirebaseInitialization() async {
    try {
      ...
      final app = await Future.any([
        Future.delayed(const Duration(seconds: 5), () => throw TimeoutException('Firebase check timeout')),
        Future(() => Firebase.app()),
      ]);
      
      print('🔥 ✅ Firebase is initialized: ${app.name}, project: ${app.options.projectId}');
    } catch (e) {
...
class _GridPageState extends State<GridPage>
  ...
  void _initializeWakeWordCallbacks() {
    ...
    _wakeWordService!.onWakeWord = (transcript) async {
      ...
      try {
        ...
        // Use ultra-fast TTS without backend/routing complexity for speed like POC
        ...
        debugPrint(
          '[TIMER] Fast announce COMPLETE at $fastAnnounceEnd (delta: ${fastAnnounceEnd - fastAnnounceStart}ms)',
        );
        ...
        // Wait for audio setup to complete (announcement already completed)
        ...
        // Brief delay to let question listening initialize before restoring notification sounds
    ...
    _wakeWordService!.onQuestion = (question) async {
      ...
      await _announceWithTimeout(
        'Okay, processing: $question. Give me a moment.',
        routing: 'system',
        speechRate: 140,
      );
    ...
    _wakeWordService!.shouldAllowWakeWordRestart = () {
      final shouldAllow = !_inQuestionMode && !_isProcessingLLM && !_isAnnouncementPlaying;
      debugPrint(
    ...
    _wakeWordService!.onStatusBarUpdate = (heardText) {
      ...
      setState(() {
        _currentlyHeard = heardText;
        _lastHeardWords = heardText;

        // Show transcription during wake word listening (whether buttons are scanning or not)
        if (_listeningForQuestion) {
          // During question processing, status updates are handled separately
          debugPrint(
            '[WakeWord] Ignoring status update during question processing: "$heardText"',
          );
        } else if (heardText.isNotEmpty) {
          // Show what's being heard during wake word detection (including during button scanning)
          _updateStatusMessageWithAutoReset(
            'Listening for wake word: "$heardText"',
            resetAfter: const Duration(seconds: 5),
          );
          debugPrint(
            '[WakeWord] Wake word service detected speech: "$heardText"',
          );
        } else if (heardText.isEmpty) {
          // Clear status when no speech is detected during wake word phase
          debugPrint('[WakeWord] No speech detected, clearing status message');
  ...
  Future<int> _getEffectivePersonalVolume() async {
    ...
    debugPrint(
      '🔊 VOLUME: Using SETTINGS personal volume: $settingsValue/10',
    );
  ...
  Future<int> _getEffectiveSystemVolume() async {
    ...
    debugPrint(
      '🔊 VOLUME: Using SETTINGS system volume: $settingsValue/10',
    );
    return settingsValue;
  }
      ...
      try {
        ...
        if (Platform.isAndroid) {
          await platform.invokeMethod('setApplicationVolume', {
            ...
            'isPersonal': !isSystemSpeaker, // true for personal volume, false for system volume
          });
        } else {
          // For iOS, we can't control system volume directly via the app
          // iOS uses hardware volume buttons and respects the user's settings
          // We just log what we would have set for debugging purposes
          debugPrint('🔊 VOLUME: iOS - Cannot programmatically set volume (security restriction). User must adjust via hardware buttons. Target would be: $targetVolume/10');
        }

        debugPrint('🔊 VOLUME: ✅ Volume configuration completed for level: $targetVolume/10');
    ...
    if (!kIsWeb && (Platform.isIOS || Platform.isAndroid)) {
      try {
        ...
        if (Platform.isAndroid) {
          ...
          // Set up optimal audio session for simultaneous recording and playback (POC approach)
          try {
            debugPrint(
              '_initializeAudioSession: Setting up optimal audio session...',
            );
            await platform.invokeMethod('setupOptimalAudioSession');
            debugPrint(
          ...
          debugPrint(
            '_initializeAudioSession: Playing silence.mp3 to warm up audio session...',
          );
          ...
          debugPrint(
            '_initializeAudioSession: iOS audio session initialized successfully',
          );
        } else {
  ...
  Future<void> _playSimpleBeep() async {
    try {
      ...
      await flutterTts.setVolume(0.7); // Moderate volume
  ...
  // Check if we should wait for switch press before starting (applies to every navigation)
    if (waitForSwitch && !isScanning && gridButtons.isNotEmpty && !_waitingForInitialSwitch) {
      debugPrint('maybeStartScanning: Waiting for switch press to begin scanning on gridpage...');
      setState(() {
  ...
  Future<void> _startAuditoryScanning() async {
    ...
    if (waitForSwitch && !_switchStartRequested) {
      debugPrint('startAuditoryScanning: Switch not pressed, blocking scanning');
      return;
    }

    // CRITICAL: Don't start scanning if we're waiting for the user to press switch
    if (_waitingForInitialSwitch) {
      debugPrint('startAuditoryScanning: Waiting for switch press, blocking scanning');
      return;
    }
    ...
    debugPrint('startAuditoryScanning: Starting first scan step...');
    _scanStep();
    debugPrint('startAuditoryScanning: Setting up periodic timer...');
  ...
  // --- Speech Recognition Debug Tracking ---
  String _currentlyHeard = ""; // What the app is currently hearing
  String _lastHeardWords = ""; // Last words detected
  ...
  Future<void> fetchGridData() async {
    ...
    try {
      ...
      final response = await cruiseShipSafeGet(
          ...
          setState(() {
            ...
            currentPageDisplayName =
                (data[0]['displayName'] ??
                        ...
                        currentPageName)
                    .toString();
            statusMessage = 'Loaded initial grid. (v1.0.2+17)';
          });
  ...
  Future<void> _speakPersonalVoice(String text) async {
    ...
    debugPrint('_speakPersonalVoice: _settingsProvider is null? ${_settingsProvider == null}');
    debugPrint('_speakPersonalVoice: _settingsProvider.settings is null? ${_settingsProvider?.settings == null}');
    ...
    // CRITICAL: Always ensure audio routing is reset to default before speaking
    // This ensures scanning announcements go to Bluetooth/default device, not built-in speaker
    if (!kIsWeb && Platform.isAndroid) {
      try {
        debugPrint(
          '_speakPersonalVoice: Resetting audio routing to default...',
        );
        final platform = MethodChannel('audio_routing');
        await platform.invokeMethod('resetToDefault');
        await Future.delayed(const Duration(milliseconds: 100));
        debugPrint('_speakPersonalVoice: Audio routing reset completed');
      } catch (e) {
        debugPrint(
    ...
    try {
      ...
      // CRITICAL: Respect user's saved personal volume (0-10 scale) instead of always using maximum
      debugPrint(
  ...
  Future<void> _speakSystemVoice(String text) async {
    ...
    try {
      ...
      // CRITICAL: Respect user's saved system volume (0-10 scale) instead of always using maximum
      // Convert systemVolume (0-10 scale) to TTS volume (0.0-1.0 scale)
      final savedSystemVolume = await _getEffectiveSystemVolume();
      final ttsVolume = (savedSystemVolume / 10.0).clamp(0.0, 1.0);
      debugPrint('_speakSystemVoice: Setting TTS volume to $ttsVolume (systemVolume: $savedSystemVolume/10)');
      await flutterTts.setVolume(ttsVolume);
      ...
      await flutterTts.speak(text);
      debugPrint('_speakSystemVoice: TTS speak() completed with volume: $ttsVolume');
    } catch (e) {
  ...
  Future<void> announceViaBackendSimple(
    ...
    try {
      ...
      // Refresh token with simple error handling
      try {
        final user = FirebaseAuth.instance.currentUser;
        if (user != null) {
  ...
  Future<void> _announceWithTimeout(
    ...
    String routing = 'system',
    int? speechRate,
    Duration timeout = const Duration(seconds: 30), // Increased from 15s to 30s for longer jokes/announcements
  }) async {
    try {
      debugPrint(
  ...
  Future<void> announceViaBackend(
    ...
    try {
      ...
      // Always refresh the token before backend call with better error handling
      try {
        final user = FirebaseAuth.instance.currentUser;
        if (user != null) {
          final refreshedToken = await user.getIdToken(true);
          if (refreshedToken != null && refreshedToken.isNotEmpty) {
            idToken = refreshedToken;
            debugPrint('announceViaBackend: Token refreshed successfully');
          } else {
            debugPrint(
              'announceViaBackend: Token refresh returned null/empty, using original token',
            );
          }
        } else {
          debugPrint(
            'announceViaBackend: No current user, using original token',
          );
        }
      } catch (e) {
        debugPrint(
          'announceViaBackend: Token refresh failed: $e, using original token',
        );
      ...
      if (response.statusCode == 200) {
        ...
        try {
          if (!kIsWeb && Platform.isIOS) {
            ...
            // Use max volume (1.0) and rely on hardware volume control
            debugPrint(
              'announceViaBackend: Setting iOS player volume to 1.0 (Hardware Control)',
            );
            ...
            debugPrint(
              'iOS: Resetting to default routing after audio complete',
            );
            await platform.invokeMethod('resetToDefault');
                ...
                final sub = player.playerStateStream.listen((state) async {
                  if (state.processingState == ProcessingState.completed) {
                    ...
                    if (duration != null && position != null) {
                      ...
                      if (remainingMs <= 200) {
                        debugPrint('Android base64: Audio truly completed (position: ${position.inSeconds}s, duration: ${duration.inSeconds}s)');
                        if (!completer.isCompleted) completer.complete();
                      } else {
                        debugPrint('Android base64: False completion detected - ${remainingMs}ms remaining, continuing playback');
                      }
                    } else {
                      // Fallback: if duration/position unavailable, trust the completed state
                      debugPrint('Android base64: Duration/position unavailable, trusting completed state');
                      if (!completer.isCompleted) completer.complete();
                    }
                  }
                });
                ...
                final sub = player.playerStateStream.listen((state) async {
                  if (state.processingState == ProcessingState.completed) {
                    ...
                    if (duration != null && position != null) {
                      ...
                      final remainingMs = duration.inMilliseconds - position.inMilliseconds;
                      if (remainingMs <= 200) {
                        debugPrint('Android audioUrl: Audio truly completed (position: ${position.inSeconds}s, duration: ${duration.inSeconds}s)');
                        if (!completer.isCompleted) completer.complete();
                      } else {
                        debugPrint('Android audioUrl: False completion detected - ${remainingMs}ms remaining, continuing playback');
                      }
                    } else {
                      // Fallback: if duration/position unavailable, trust the completed state
                      debugPrint('Android audioUrl: Duration/position unavailable, trusting completed state');
                      if (!completer.isCompleted) completer.complete();
                    }
                  }
                });
      ...
      // After using forceSpeaker, reset to default device with additional restoration for personal audio
      if (!kIsWeb && (Platform.isIOS || Platform.isAndroid)) {
        try {
          final platform = MethodChannel('audio_routing');
          ...
          // Simple delay to let the audio session settle
          if (Platform.isIOS) {
            await Future.delayed(const Duration(milliseconds: 200));
            debugPrint('iOS: Audio routing restored to default/Bluetooth');
          }
        } catch (e) {
  ...
  Future<void> handleButtonAction(Map<String, dynamic> buttonData) async {
    ...
    if (llmQuery != null && llmQuery.isNotEmpty) {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => FreestylePage(
              ...
              originatingButtonText: originatingButtonText,
            ),
          ),
        );
        return;
      ...
      if (buttonData['llmSpecial'] != 'goBack' &&
        ...
        // Capture question for chat history before clearing it
        final currentQuestion = questionDisplay.isNotEmpty
            ? questionDisplay
            : '';
        ...
        debugPrint(
          'Restoring previous grid/page after LLM option. previousPageName: $previousPageName, previousGridButtons: ${previousGridButtons != null}',
        );
        if (previousPageName != null) {
          final pageToRestore = previousPageName;
          previousPageName = null;
      ...
      try {
        ...
        final response = await http.post(
          Uri.parse('${EnvironmentConfig.apiBaseUrl}/get-current-events'),
          headers: {
            'Authorization': 'Bearer ${widget.idToken}',
            'X-User-ID': widget.aacUserId,
            'Content-Type': 'application/json',
  ...
  Future<void> fetchGridDataForPage(
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('${EnvironmentConfig.apiBaseUrl}/pages?name=$pageName'),
        headers: {
          'Authorization': 'Bearer ${widget.idToken}',
          'X-User-ID': widget.aacUserId,
          'Content-Type': 'application/json',
      ...
      if (response.statusCode == 200) {
        ...
        if (matchedPage != null && matchedPage['buttons'] != null) {
          // --- Patch: Filter, sort, and lay out grid buttons ---
          final settingsProvider = Provider.of<UserSettingsProvider>(
            context,
            listen: false,
          );
          ...
          setState(() {
            ...
            // Reset wait-for-switch flag when navigating to new page
            _waitingForInitialSwitch = false;

            // Update navigation history if addToHistory is true
            if (addToHistory) {
              // Don't add duplicate if we're already on this page
          ...
          // Don't start scanning here - let the calling code control when to start scanning
        } else {
          setState(() {
            gridButtons = [];
                ...
                children: [
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            if (_microphoneListening) const SizedBox(width: 8),
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              ...
                              width: _highlightQuestionBox ? 2 : 1,
                            ),
                            boxShadow: [
                              BoxShadow(
                                color: Colors.black.withOpacity(0.05),
                  ...
                  if (statusMessage != null && statusMessage!.isNotEmpty)
                    statusMessage!.startsWith('❌')
                        ? // Error banner: full-width red bar across bottom
                          Container(
                            width: double.infinity,
                            padding: const EdgeInsets.symmetric(
                              vertical: 12.0,
